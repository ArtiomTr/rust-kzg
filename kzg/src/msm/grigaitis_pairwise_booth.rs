...
use crate::{
    msm::{
        ...
        pippenger_utils::{p1_dadd, p1_to_jacobian, type_is_zero},
    },
    ...
    > GrigaitisTable<TFr, TG1, TG1Fp, TG1Affine, TG1ProjAddAffine>
{
    ...
    pub fn multiply_batch(&self, scalars: &[Vec<TFr>]) -> Vec<TG1> {
        ...
        #[cfg(not(feature = "parallel"))]
        {
            ...
            let mut buckets = vec![Default::default(); 1 << (window - 1)];
    ...
    fn multiply_sequential_raw(
        ...
        let chunk_size = numpoints * ext + (numpoints * ext) * (numpoints * ext - 1);
        ...
        loop {
            ...
            let curr_chunk_size =
                numpoints * batches + (numpoints * batches) * (numpoints * batches - 1);
            let pointset =
        ...
        let pointset =
            &points[0..(numpoints * batches + (numpoints * batches) * (numpoints * batches - 1))];
        p1_tile_bgmw(
            pointset, scalars, buckets, 0, wbits, cbits, numpoints, batches, 0,
        );
        strauss_booth_decode_end(buckets, pointset);
        ...
        p1_integrate_buckets(&mut ret, &points, wbits - 1);
...
fn strauss_booth_decode<TG1: G1, TFp: G1Fp, TG1Affine: G1Affine<TG1, TFp>>(
    ...
    if booth_idx != 0 {
        ...
        let comb_index = numpoints
            + first_index * (2 * numpoints - 1 - first_index)
