    ...
    > GrigaitisTable<TFr, TG1, TG1Fp, TG1Affine, TG1ProjAddAffine>
{
    pub fn new(points: &[TG1], matrix: &[Vec<TG1>]) -> Result<Option<Self>, String> {
        ...
        for p in points {
            ...
            for _ in 1..window {
                temp.dbl_assign();
                table.push(temp.clone());
            }
        }

        let extended_point_length = points.len() * window;

        let ext_param: usize = option_env!("EXT").map(|v| v.parse().unwrap()).unwrap_or(1);

        for j in 0..(ext_param - 1) {
            for i in j * extended_point_length..(j + 1) * extended_point_length {
        ...
        for i in 0..(extended_point_length * ext_param) {
            for j in i + 1..(extended_point_length * ext_param) {
                table.push(table[i].add_or_dbl(&table[j]));
                table.push(table[i].sub(&table[j]));
            }
        }

        let ext_point_len = extended_point_length * ext_param;
        let chunk_size = ext_point_len + (ext_point_len) * (ext_point_len - 1);
        ...
        let pts = extended_point_length * (h % ext_param);
        ...
        let last_chunk_len = pts + (pts) * pts.saturating_sub(1);
        assert_eq!(table.len(), chunk_size * (h / ext_param) + last_chunk_len);

        let table = batch_convert(&table);
        println!(
            "grigaitis_pairwise_cpdlh table size in bytes: {}",
    ...
    pub fn multiply_batch(&self, scalars: &[Vec<TFr>]) -> Vec<TG1> {
        ...
        #[cfg(not(feature = "parallel"))]
        {
            ...
            let mut buckets = vec![Default::default(); 1 << (window - 2)];
    ...
    fn multiply_sequential_raw(
        ...
        let numpoints_total = numpoints * window * ext;
        let chunk_size = numpoints_total + (numpoints_total) * (numpoints_total - 1);
        ...
        loop {
            ...
            let curr_chunk_size = numpoints * window * batches
                + (numpoints * window * batches) * (numpoints * window * batches - 1);
            let pointset =
        ...
        let pointset = &points[0..(numpoints * window * batches
            + (numpoints * window * batches) * (numpoints * window * batches - 1))];
        p1_tile_bgmw(
            window, pointset, scalars, buckets, 0, wbits, cbits, numpoints, batches, 0,
        );
        strauss_booth_decode_end(buckets, pointset);
        ...
        p1_integrate_buckets(&mut ret, &points, wbits - 2);
...
fn strauss_booth_decode<TG1: G1, TFp: G1Fp, TG1Affine: G1Affine<TG1, TFp>>(
    ...
    booth_idx &= (1 << wbits) - 1;
    if booth_idx != 0 {
        let offsetted_by = booth_idx.trailing_zeros() as usize;
        let bucket_index = ((booth_idx >> offsetted_by) / 2) as usize;

        let point_index = point_index * window + offsetted_by;

        let (sum, saved) = &mut buckets[bucket_index];

        let Some(saved_i) = saved else {
            *saved = Some(point_index | ((booth_sign as usize) << (usize::BITS - 1)));
            return;
        };

        let saved_sign = (*saved_i >> (usize::BITS - 1)) as u64;
        let saved_index = *saved_i & ((1 << (usize::BITS - 1)) - 1);

        let offset = booth_sign ^ saved_sign; // this will be 1 when we need to take a-b instead of a+b from precomputations

        let (first_index, second_index, neg) = if point_index > saved_index {
            (saved_index, point_index, saved_sign != 0)
        } else {
            (point_index, saved_index, booth_sign != 0)
        };

        let comb_index = (numpoints * window)
            + first_index * (2 * numpoints * window - 1 - first_index)
...
fn strauss_booth_decode_end<TG1: G1, TFp: G1Fp, TG1Affine: G1Affine<TG1, TFp>>(
    ...
    for (sum, index) in buckets {
        if let Some(index) = index {
            let sign = (*index >> (usize::BITS - 1)) != 0;
            let index = *index & ((1 << (usize::BITS - 1)) - 1);

            if sign {
                sum.push(points[index].neg());
            } else {
